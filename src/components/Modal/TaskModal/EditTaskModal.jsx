...
const EditTaskModal = () => {
  ...
  const [hoursRequired, setHoursRequired] = useState(task.hoursRequired || 0)
  // Keep the original creation date or set to current date if not available
  ...
  const handleSubmit = (e) => {
    ...
    if (hoursRequired < 0 || isNaN(hoursRequired)) {
      setErrorHours(true)
      error = true
    } else {
      setErrorHours(false)
    ...
    <Backdrop onClick={() => dispatch(closeAllModals())} mobile={false}>
      <form onClick={handleSubmit}>
        ...
        >
          ...
          <div className='modal__input-container'>
            <h3 className='modal-label'>Hours Required</h3>
            <input
              type='number'
              min='0'
              value={hoursRequired}
              ...
              className={`modal-input ${errorHours && 'modal-input__error'}`}
            />
            {errorHours && (
          ...
          {/* Creation Date Input */}
          <div className='modal__input-container'>
            <h3 className='modal-label'>Creation Date</h3>
            <input
              type='date'
              value={formatDateForInput(creationDate)}
              onChange={(e) => {
                // Convert the date to ISO string format
                const newDate = new Date(e.target.value)
                setCreationDate(newDate.toISOString())
              }}
